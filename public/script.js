...
import {
    ...
    abilityDefinitions,
    featDefinitions,
} from "./game-logic/class-progression.js";
import {
    ItemGenerator,
    ItemManager,
    itemCategories,
...
function validateAndFixStats(player) {
    ...
    if (!player.stats) {
        ...
        statNames.forEach((statName) => {
            if (
                typeof player.stats[statName] !== "number" ||
                ...
                statsFixed = true;
                console.log(
                    `Fixed ${statName}: ${oldValue} -> ${minStatValue}`,
                );
            }
        });
    }
...
function autoFixStatsInStorage() {
    try {
        ...
        if (player && player.name) {
            const progressionData = localStorage.getItem(
                `progression_${player.name}`,
            );
            if (progressionData) {
                ...
                if (progData.stats) {
                    ...
                    if (validateAndFixStats(tempPlayer)) {
                        ...
                        console.log(
                            "Fixed stats in character progression data",
                        );
                    }
                }
            }
...
function showCountryDetails(countryKey) {
    const country = countries[countryKey];
    const countryCities = Object.values(cities).filter(
        (city) => city.country === countryKey,
    );
    const countryRegions = Object.values(regions).filter(
        (region) => region.country === countryKey,
    );
...
async function addToConversationHistory(role, content) {
    ...
    if (conversationHistory.messages.length > conversationHistory.maxMessages) {
        conversationHistory.messages = conversationHistory.messages.slice(
...
async function generateAndDisplaySceneryImage() {
    const illustrationModeBtn = document.getElementById(
        "illustration-mode-btn",
    );
    ...
    if (
        !sceneryPrompt ||
        sceneryPrompt === "A mysterious and intriguing landscape."
    ) {
    ...
    if (!player.portraitUrl || player.portraitUrl.trim() === "") {
        displayMessage(
            "Character portrait not found. Cannot generate scenery image.",
            "error",
        );
    ...
    const characterSeed = player.name
        ? player.name
              .toLowerCase()
              .replace(/[^a-z0-9]/g, "")
              .split("")
    ...
    try {
        const response = await fetch(
            "https://ainovel.site/api/generate-image",
            {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    negative_prompt:
                        "text, watermark, signature, poorly drawn character, disfigured character, ugly, blurry character",
        ...
        if (!response.ok) {
            ...
            displayMessage(
                `Image generation service returned an error: ${response.status}.`,
                "error",
            );
            // Remove loading indicator
            return;
        ...
        if (result.imageUrl) {
            ...
            console.error(
                "Image generation failed or did not return an image URL. Response:",
                result,
            );
            displayMessage(
                "Image generation failed or did not return an image URL.",
                "error",
            );
        }
    } catch (error) {
...
Image Prompt:`;
    ...
    try {
        ...
        if (imagePrompt && imagePrompt.trim() !== "") {
            ...
            console.error(
                "Gemini API returned empty or no text for image prompt.",
            );
...
async function processAlignmentChange(change) {
    ...
    try {
        ...
        if (!result || typeof result.newType !== "string" || !result.newType) {
            console.error(
                "Invalid object returned from AlignmentSystem.updateAlignment:",
                result,
            );
        ...
        if (result.changed) {
            const changeText =
                (change.good && change.good > 0) ||
                (change.law && change.law > 0)
                    ? "improved"
                    : "declined";
            const newAlignmentName = result.newType;

            displayMessage(
                `Your moral standing has ${changeText}. You are now ${newAlignmentName}.`,
        ...
        console.error(
            "An error occurred inside the processAlignmentChange function:",
            e,
        );
    }
...
function updateRelationship(
    ...
    if (!player.relationships[npcName]) {
        ...
        player.relationships[npcName] = {
            ...
            description:
                npcDescription ||
                "A person you've encountered in your travels.",
            firstMeeting: new Date().toLocaleDateString(),
        };
        // This message will now only appear when intended
        displayMessage(
            `You've established a new relationship with ${npcName}.`,
            "success",
        );
    }
    ...
    const modifiedTrustChange =
        trustChange + (alignmentModifier.npcTrustModifier || 0) * 0.1;
    relationship.trust = Math.max(
        0,
        Math.min(100, relationship.trust + modifiedTrustChange),
    );
    ...
    if (oldStatus !== relationship.status) {
        displayMessage(
            `Your relationship with ${npcName} is now ${relationship.status}.`,
            "info",
        );
    }
...
function fixCharacterStats() {
    ...
    if (
        !confirm(
            `Fix stats for ${player.name} (Level ${player.level} ${player.class})? This will recalculate all stats based on level and class.`,
        )
    ) {
        return 0;
    }
    ...
    if (
        ...
        player.classProgression.class
    ) {
        const altClassData = window.classProgression
            ? window.classProgression[
                  player.classProgression.class.toLowerCase()
              ]
            : null;
    ...
    console.log(
        `Fixing stats for ${player.name}: Level ${player.level}, Class: ${player.class}, Primary Stats: [${primaryStats.join(", ")}]`,
    );
    ...
    allStatNames.forEach((statName) => {
        ...
        if (originalValue !== correctValue) {
            ...
            console.log(
                `${statName}: ${originalValue} → ${correctValue} (Primary: ${primaryStats.includes(statName)})`,
            );
            changedStats++;
        }
    });
    ...
    if (changedStats > 0) {
        displayMessage(
            `✅ Fixed ${changedStats} stats for ${player.name}!`,
            "success",
        );

        if (primaryStats.length > 0) {
            displayMessage(
                `Primary stats: ${primaryStats.join(", ")} (+2 base, +2 every 4 levels)`,
                "info",
            );
        } else {
            displayMessage(
                `Warning: Could not determine primary stats for class '${player.class}'`,
                "error",
            );
        }

        displayMessage(
            `All stats: ${allStatNames.map((s) => `${s}: ${player.stats[s]}`).join(", ")}`,
            "info",
        );
        ...
        if (
            ...
            typeof CharacterManager.saveProgression === "function"
        ) {
        ...
        displayMessage(
            "✅ All stats are already correct for this character's level and class.",
            "info",
        );
    }
...
async function checkNPCMentionsAndAdd(
    aiResponse,
    playerCommand,
    gameContextPlayer,
) {
    console.log(
        "checkNPCMentionsAndAdd: Scanning AI response for NPC mentions...",
    );

    // Ensure the RelationshipMiddleware is available on the window object
    if (!window.RelationshipMiddleware) {
        console.error(
            "RelationshipMiddleware is not available. Cannot process NPC mentions.",
        );
        return;
    }
    ...
    try {
        ...
        const npcNames = await RelationshipMiddleware.extractNPCNames(
            aiResponse,
            gameContextPlayer,
        );
        ...
        if (npcNames.length === 0) {
            console.log(
                "checkNPCMentionsAndAdd: No NPC names were identified in the AI response.",
            );
            return;
        }

        console.log(
            "checkNPCMentionsAndAdd: Identified potential NPCs:",
            npcNames,
        );

        for (const npcName of npcNames) {
            // Resolve the NPC's canonical identity and get a description for them
            const identity = await RelationshipMiddleware.resolveNpcIdentity(
                npcName,
                gameContextPlayer.relationships || {},
                playerCommand,
                aiResponse,
            );
            ...
            if (identity.description === "Not an NPC.") {
                console.log(
                    `checkNPCMentionsAndAdd: Skipping relationship creation for "${npcName}" as it was identified as a non-NPC entity.`,
                );
                continue;
            }

            // Check if this NPC already exists in the current location's NPC list
            const existingNPCsInLocation = getNPCsInLocation(
                gameContextPlayer.currentLocation,
            );
            const npcAlreadyInLocation = existingNPCsInLocation.some(
                (npc) => npc.name === identity.canonicalName,
            );
...
const donationModalCloseBtn = document.getElementById(
...
const characterCreationScreen = document.getElementById(
...
const generateBackgroundBtn = document.getElementById(
...
const npcClasses = {
    warrior: {
        ...
        damage: "1d8",
        ac_bonus: 3,
        description: "A sturdy frontline fighter.",
    },
    mage: {
        hp_base: 115,
        hp_per_level: 4,
    ...
    rogue: {
        ...
        damage: "1d6",
        ac_bonus: 1,
        description: "A nimble and cunning scout.",
    },
    healer: {
        hp_base: 118,
        hp_per_level: 5,
        damage: "1d4",
        ac_bonus: 0,
        description: "A supportive divine spellcaster.",
        skills: ["Minor Heal"],
    },
    // Default fallback
    commoner: {
...
async function generateNpcStats(npcName, playerLevel) {
    ...
    try {
        ...
        console.error(
            "AI failed to assign NPC class, using default. Error:",
            error,
        );
    }
...
function updateGold(amount, reason = "") {
    ...
    console.log(
        `updateGold: Amount: ${amount}, Reason: ${reason}, Old Gold: ${oldGold}, New Gold: ${player.gold}`,
    ); // ADD THIS LOG

    if (amount > 0) {
        displayMessage(
            `You gained ${Number(amount)} gold${reason ? ` (${reason})` : ""}!`,
            "success",
        );
    } else if (amount < 0) {
        const actualLoss = oldGold - player.gold; // Recalculate actual loss after Math.max(0,...)
        displayMessage(
            `You lost ${Math.abs(Number(amount))} gold${reason ? ` (${reason})` : ""}!`,
            "error",
        );
    }
    ...
    if (
        window.ItemManager &&
        typeof ItemManager.saveInventoryToStorage === "function"
    ) {
    ...
    console.log(
        `updateGold: About to call saveGame(). Current player.gold is ${player.gold}`,
    ); // ADD THIS LOG
    saveGame();
}
...
function displayMessage(message, type = "info") {
    ...
    if (
        ...
        message.includes("Your alignment shifts. undefined")
    ) {
        console.error("------------------------------------------------------");
        console.error(
            "BUG DETECTED! The broken alignment message was passed to displayMessage.",
        );
        console.error(
            "The call stack below shows exactly which function is responsible.",
        );
...
function updatePlayerStatsDisplay() {
    ...
    console.log(
        "[MAIN SCRIPT] Current player location:",
        player ? player.currentLocation : "player undefined",
    );
    ...
    requestAnimationFrame(() => {
        ...
        setTimeout(() => {
            playerNameDisplay.textContent = displayText;
            console.log(
                `[MAIN SCRIPT] Final player name display content: "${playerNameDisplay.textContent}"`,
            );
            console.log(
                `[MAIN SCRIPT] Final player name display innerHTML: "${playerNameDisplay.innerHTML}"`,
            );
        }, 50);
    });
...
function hideScreen(screenId) {
    const screens = [
        ...
        questInterface,
        backgroundInterface,
        document.getElementById("progression-interface"),
    ];
    screens.forEach((screen) => {
        if (screen.id === screenId) {
            screen.classList.add("hidden");
...
function saveGame() {
    if (
        !confirm(
            "Are you sure you want to save your game? This will overwrite any existing save.",
        )
    ) {
        return;
    }
    ...
    ); // ADD THIS (JSON.parse(JSON.stringify()) for a clean clone log)
...
function loadGame() {
    ...
    if (savedGame) {
        console.log(
            "loadGame: Raw data from localStorage ('pedenaRPGSave'):",
            savedGame.substring(0, 500) + "...",
        ); // Log first 500 chars
        const saveData = JSON.parse(savedGame);
        console.log(
            "loadGame: Parsed saveData.player.gold from localStorage:",
            saveData.player
                ? saveData.player.gold
                : "N/A - saveData.player not found",
        );
        ...
        if (saveData.player) {
            ...
            console.log(
                `[SCRIPT.JS] After loadGame, window.player HP: <span class="math-inline">${window.player.hp}/${window.player.maxHp}</span>`,
            ); // Should be 140/140
            console.log(
                `[SCRIPT.JS] After loadGame, is module player === window.player? ${player === window.player}`,
            ); // Should be true
            ...
            if (saveData.gameWorld) {
                ...
                gameWorld.locationMemory = new Map(
                    saveData.gameWorld.locationMemory || [],
                );
                gameWorld.lastNPCInteraction =
                    saveData.gameWorld.lastNPCInteraction || null;

                // FIX: Check for the new properties and initialize them if they don't exist in the save file.
                gameWorld.time = saveData.gameWorld.time
                    ? new Date(saveData.gameWorld.time)
                    : new Date(864, 5, 12, 8, 0, 0);
        ...
        console.log(
            "loadGame: Global player.gold immediately after assignment from saved data:",
            player.gold,
        );
        ...
        if (validateAndFixStats(player)) {
            displayMessage(
                "Character stats have been automatically corrected.",
                "info",
            );
        ...
        console.log(
            "loadGame: Final player.gold after all loading steps:",
            player.gold,
        );

        console.log(
            `[SCRIPT.JS] loadGame: player HP set to <span class="math-inline">${player.hp}/${player.maxHp}</span>`,
        );
        console.log(
            `[SCRIPT.JS] loadGame: window.player HP is now <span class="math-inline">${window.player.hp}/${window.player.maxHp}</span>`,
        );
        console.log(
            `[SCRIPT.JS] loadGame: is module player === window.player? ${player === window.player}`,
        ); // Should be true
        ...
        displayMessage(
            `Welcome back, ${player.name}! You are in ${player.currentLocation}.`,
        );
            ...
            if (firstBrace !== -1 && lastBrace > firstBrace) {
                ...
                if (jsonString.startsWith("{") && jsonString.endsWith("}")) {
                    try {
                        ...
                        console.warn(
                            "Found what looked like JSON, but it failed to parse. Returning raw text.",
                            e,
                        );
                    }
                }
...
function saveNPCToLocation(npc, location) {
    ...
    if (existingIndex >= 0) {
        ...
        console.log(
            `saveNPCToLocation: Updated existing NPC "${npc.name}" at "${location}"`,
        );
    } else {
        locationNPCs.push(npc); // Add new
        console.log(
            `saveNPCToLocation: Added new NPC "${npc.name}" to location "${location}"`,
        );
    }
}
...
Make the location name proper and descriptive.`;
        ...
        try {
            ...
            if (
                typeof movementData.canMove !== "boolean" ||
            ...
            movementData.encounterChance = Math.max(
                0,
                Math.min(1, movementData.encounterChance),
            );
            movementData.goldCost = Math.max(
                0,
                parseInt(movementData.goldCost) || 0,
            );
            movementData.newLocation = movementData.newLocation.trim();
        } catch (parseError) {
            console.error(
                "Failed to parse movement JSON:",
                parseError,
                "Raw response:",
                response,
            );
            ...
            if (sanitizedDestination) {
                const fallbackLocation = sanitizedDestination.replace(
                    /\b\w/g,
        ...
        if (!movementData.canMove) {
            displayMessage(
                movementData.description ||
                    "You cannot travel there right now.",
                "error",
            );
            return;
        }
        ...
        if (movementData.goldCost > 0) {
            if (player.gold < movementData.goldCost) {
                displayMessage(
                    `You need ${movementData.goldCost} gold to travel there, but you only have ${player.gold} gold.`,
                    "error",
                );
                return;
            }
        ...
        if (LocationManager && LocationManager.saveLocationToHistory) {
            LocationManager.saveLocationToHistory(
        ...
        addToConversationHistory(
            "assistant",
            `${player.name} traveled from ${oldLocation} to ${movementData.newLocation}`,
        );

        // Check for encounters based on AI decision
        if (
            movementData.encounterChance > 0 &&
            Math.random() < movementData.encounterChance
        ) {
            setTimeout(async () => {
                ...
                const encounterFunction =
                    encounterTypes[movementData.encounterType] ||
        ...
        displayMessage(
            "There was an issue processing your movement. Please try again.",
            "error",
        );
    }
}
...
async function handleMovement(command) {
    ...
    if (result.success) {
        ...
        if (isMultiplayerMode) {
            multiplayerClient.requestTravel(
                result.newLocation,
                result.description,
            );
            multiplayerClient.sendAction(
                `traveled to ${result.newLocation}`,
                result.description,
            );
        }
...
async function generateRandomEncounter() {
    ...
    try {
        ...
        if (!jsonMatch) {
            throw new Error(
                "AI response for random encounter did not contain valid JSON.",
            );
        }
        ...
        console.error(
            "Error during AI-powered random encounter generation:",
            error,
        );
        displayMessage(
            "The path ahead is quiet, the world holding its breath for a moment.",
            "info",
        ); // Graceful fallback.
    }
}
...
async function generateAndInitiateCombatEncounter(enemySuggestion = null) {
    ...
    const suggestionText = enemySuggestion
        ? `The player's action suggested an encounter with: "${enemySuggestion}". Use this as creative inspiration.`
        : "The encounter is random, based on the location.";
    ...
    **IMPORTANT**: Do NOT name the enemy "ambush" or use the raw suggestion as the name. The name must be a creative, specific creature or person.
    ...
    try {
        ...
        if (!jsonMatch) {
            throw new Error(
                "AI response for enemy generation did not contain valid JSON.",
            );
        }
        ...
        if (
            ...
            throw new Error(
                "AI response for enemy was missing required fields (name, hp, attack).",
            );
        }
        ...
        await CombatSystem.initiateCombat(
            player,
            enemy,
...
async function generateTreasureEncounter() {
    ...
    switch (treasureType) {
        case "gold":
            const goldAmount =
                Math.floor(Math.random() * (player.level * 30)) + 20;
            updateGold(goldAmount, "treasure found");
            displayMessage(
                `💰 You found ${goldAmount} gold coins hidden in the area!`,
                "success",
            );
            break;
        ...
        case "item":
            if (
                window.ItemManager &&
                typeof ItemManager.generateItem === "function"
            ) {
        ...
        case "multiple":
            const smallGold =
                Math.floor(Math.random() * (player.level * 15)) + 10;
            updateGold(smallGold, "treasure cache");
            displayMessage(
                `💰 You found a small treasure cache with ${smallGold} gold!`,
                "success",
            );

            if (Math.random() > 0.5) {
                displayMessage(
                    "🍺 You also found a healing potion!",
                    "success",
                );
                if (!player.inventory) player.inventory = [];
                player.inventory.push({
...
async function handleItemEnhancement(command, lowerCommand) {
    ...
    const usedKeyword = enhanceKeywords.find((keyword) =>
    ...
    const itemNameMatch = command.match(
        new RegExp(
            `${usedKeyword}\\s+(?:the\\s+|my\\s+)?(.+?)(?:\\s+(?:with|using|at)\\s+|$)`,
            "i",
        ),
    );
    if (!itemNameMatch) {
        displayMessage(
            "What item would you like to enhance? Please specify an item name.",
            "error",
        );
        return { handled: true };
    }
    ...
    if (itemIndex === -1) {
        displayMessage(
            `You don't have "${itemName}" in your inventory to enhance.`,
            "error",
        );
        return { handled: true };
    }
    ...
    if (!canItemBeEnhanced(item)) {
        displayMessage(
            `${item.name} cannot be enhanced further or is not enhanceable.`,
            "error",
        );
        return { handled: true };
    }
    ...
    if (!hasEnhancementResources(enhancementCost)) {
        displayMessage(
            `You need ${enhancementCost.gold} gold and ${enhancementCost.materials.join(", ")} to enhance ${item.name}.`,
            "error",
        );
        return { handled: true };
    }
    ...
    // Perform enhancement
    const enhancementResult = performItemEnhancement(
        item,
        enhancementType,
        enhancementCost,
    );
    ...
    if (enhancementResult.success) {
        ...
        displayMessage(
            `Successfully ${usedKeyword}ed ${item.name}! ${enhancementResult.description}`,
            "success",
        );
        ...
        const inventoryInterface = document.getElementById(
            "inventory-interface",
        );
        if (
            inventoryInterface &&
            !inventoryInterface.classList.contains("hidden")
        ) {
            displayInventory();
        }
        ...
        displayMessage(
            `Failed to enhance ${item.name}: ${enhancementResult.reason}`,
            "error",
        );
    }
...
function performItemEnhancement(item, enhancementType, cost) {
    ...
    switch (enhancementType) {
        ...
        case "stat_boost":
            ...
            if (newItem.effects) {
                newItem.effects.push(
                    `enhancement_level_${newItem.enhancementLevel}`,
                );
            } else {
                newItem.effects = [
                    `enhancement_level_${newItem.enhancementLevel}`,
        ...
        case "magical_enhancement":
            ...
            const randomEffect =
                magicalEffects[
                    Math.floor(Math.random() * magicalEffects.length)
                ];
            if (newItem.effects) {
...
async function checkRelationshipChanges(playerCommand, aiResponse) {
    ...
    npcNames.forEach((npcName) => {
        ...
        const relationship = updateRelationship(
            ...
            0,
            0,
            `Encountered in ${player.currentLocation}.`,
        );
        if (relationship) {
...
function debugInventory() {
    ...
    console.log(
        "Saved inventory in localStorage:",
        savedInventory ? JSON.parse(savedInventory) : null,
    );
    ...
    if (savedGame) {
        const saveData = JSON.parse(savedGame);
        console.log(
            "Player inventory in main save:",
            saveData.player ? saveData.player.inventory : null,
        );
        console.log(
            "Player gold in main save:",
            saveData.player ? saveData.player.gold : null,
        );
    }
...
function fixInventory() {
    ...
    if (
        window.ItemManager &&
        typeof ItemManager.loadInventoryFromStorage === "function"
    ) {
        ItemManager.loadInventoryFromStorage(player);
    }
    ...
    if (
        !document
            .getElementById("inventory-interface")
    ...
    displayMessage(
        `Fixed inventory: removed ${removedDuplicates} duplicates. You now have ${player.inventory.length} items.`,
        "success",
    );
    updatePlayerStatsDisplay();
}
...
function cleanupRelationships() {
    ...
    if (removedCount > 0) {
        displayMessage(
            `🧹 Cleaned up ${removedCount} invalid relationship entries.`,
            "success",
        );
        saveGame();

        // Refresh background display if it's open
        const backgroundInterface = document.getElementById(
            "background-interface",
        );
        if (
            backgroundInterface &&
            !backgroundInterface.classList.contains("hidden")
        ) {
            displayCharacterBackground();
        }
    } else {
        displayMessage(
            "✅ No invalid relationships found to clean up.",
            "info",
        );
    }
}
...
function resetCharacterProgression() {
    ...
    if (
        !confirm(
            "Are you sure you want to reset your character progression? This will rebuild your abilities, spells, and feats based on your current level and class.",
        )
    ) {
        return;
    }
    ...
    try {
        ...
        if (
            CharacterManager &&
            typeof CharacterManager.initializeCharacter === "function"
        ) {
            ...
            displayMessage(
                "✅ Character progression reset successfully!",
                "success",
            );
            displayMessage(
                `Reinitialized as Level ${currentLevel} ${currentClass} with updated abilities and spells.`,
                "info",
            );
            ...
            const progressionInterface = document.getElementById(
                "progression-interface",
            );
            if (
                progressionInterface &&
                !progressionInterface.classList.contains("hidden")
            ) {
                displayCharacterProgression();
            }
        } else {
            displayMessage(
                "CharacterManager not available for progression reset.",
                "error",
            );
        }
    } catch (error) {
        ...
        displayMessage(
            "Error occurred while resetting progression. Check console for details.",
            "error",
        );
    }
}
...
async function generateEventEncounter() {
    ...
    switch (eventType) {
        case "weather_change":
            ...
            displayMessage(
                `🌤️ The weather ${weather}, affecting your journey.`,
                "info",
            );
            break;
        ...
        case "mysterious_sign":
            displayMessage(
                "🪧 You find a weathered signpost with mysterious directions carved into it.",
                "exploration",
            );
            displayMessage(
                "The ancient words seem to point toward hidden paths and secret locations.",
                "info",
            );
            break;
        ...
        case "helpful_spirit":
            ...
            displayMessage(
                "👻 A benevolent spirit appears and blesses you with healing energy!",
                "success",
            );
        ...
        case "ancient_relic":
            displayMessage(
                "🏛️ You discover the ruins of an ancient structure, worn down by time.",
                "exploration",
            );
            displayMessage(
                "Though mostly destroyed, you sense this place once held great significance.",
                "info",
            );
            break;
        ...
        case "crossroads_choice":
            displayMessage(
                "🛤️ You reach a crossroads with multiple paths branching in different directions.",
                "exploration",
            );
            displayMessage(
                "Each path seems to lead to different adventures and opportunities.",
                "info",
            );
            break;
    }
...
async function generateQuest() {
    ...
    try {
        ...
        displayMessage(
            `Rewards: ${quest.rewards.gold} gold, ${quest.rewards.experience} XP${quest.rewards.items.length > 0 ? ", " + quest.rewards.items.join(", ") : ""}`,
            "info",
        );
        displayMessage(
            `Difficulty: ${quest.difficulty} | Estimated Time: ${quest.estimatedTime}`,
            "info",
        );
        ...
        displayMessage(
            `Quest Giver: ${quest.questGiver} | Location: ${quest.location}`,
            "info",
        );
        ...
        displayMessage(
            "Unable to generate quest at this time. Please try again.",
            "error",
        );
        // Try fallback
    ...
    player.quests.forEach((quest) => {
        if (!quest.completed) {
            ...
            for (const pattern of actionPatterns) {
                const hasQuestKeyword = pattern.keywords.some((keyword) =>
                    questText.includes(keyword),
                );
                const hasActionTrigger = pattern.triggers.some((trigger) =>
                    actionText.includes(trigger),
                );
            ...
            if (questTargets.length > 0) {
                const targetMatches = questTargets.filter((target) =>
                    actionTargets.includes(target),
                );
            ...
            if (quest.location) {
                ...
                const actionRequiresLocation = [
                    ...
                    "visit",
                    "explore",
                ].some((verb) => actionText.includes(verb));

                if (
                    (questLocation.includes(currentLocation) ||
                        currentLocation.includes(questLocation)) &&
                    actionRequiresLocation
                ) {
                    completionScore += 1;
                    console.log("Location-based progress:", {
                        questLocation,
                        currentLocation,
                        actionRequiresLocation,
                    });
            ...
            if (quest.title) {
                ...
                if (
                    titleMatches.length >=
                    Math.max(2, Math.ceil(titleWords.length * 0.6))
                ) {
                    completionScore += 2;
                    console.log("Quest title substantial match:", {
                        titleWords,
                        titleMatches,
                        actionText,
                    });
            ...
            const hasExplicitCompletion = explicitCompletionPhrases.some(
                (phrase) =>
                    actionText.includes(phrase) ||
                    questText.includes("completed"),
            );

            // Complete quest if score meets threshold OR explicit completion detected
            isCompleted =
                completionScore >= requiredScore || hasExplicitCompletion;
            ...
            if (isCompleted) {
                ...
                displayMessage(
                    `🎉 Quest completed: ${quest.title || "Unknown Quest"}!`,
                    "success",
                );
                ...
                if (quest.rewards && typeof quest.rewards === "object") {
                    ...
                    xpAwarded =
                        parseInt(quest.rewards.experience) ||
                        parseInt(quest.rewards.exp) ||
                        parseInt(quest.rewards.xp) ||
                        0;
                ...
                if (goldAwarded === 0) {
                    ...
                    goldAwarded = Math.floor(
                        (50 + player.level * 25) * difficultyMultiplier,
                    );
                }
                ...
                displayMessage(
                    `⭐ You gained ${xpAwarded} experience!`,
                    "success",
                );
...
function manualCompleteQuest(questTitle) {
    ...
    if (!quest) {
        displayMessage(
            `Quest "${questTitle}" not found in your active quests.`,
            "error",
        );
        return;
    }
    ...
    addToConversationHistory(
        "assistant",
        `${player.name} completed the quest: ${quest.title}`,
    );
}
...
function gainExperience(amount) {
    ...
    console.log(
        `[script.js] gainExperience: Player XP after update: ${player.exp}`,
    );
    ...
    while (player.exp >= player.expToNextLevel) {
        ...
        console.log(
            `[script.js] gainExperience: Player leveled up to ${player.level}. New XP: ${player.exp}, New XP to Next: ${player.expToNextLevel}`,
        );

        // Apply class progression if available
        if (
            window.CharacterManager &&
            typeof CharacterManager.levelUp === "function"
        ) {
...
async function enemyAttack() {
    ...
    if (player.stats && player.stats.constitution) {
        const constitutionBonus = Math.floor(
            (player.stats.constitution - 10) / 2,
        );
    ...
    displayMessage(
        `${enemy.name} attacks you for ${finalDamage} damage!${playerDefense > 0 ? ` (${totalDamage} reduced by ${playerDefense} defense)` : ""}`,
        "combat",
    );
    displayMessage(
        `Your HP: ${player.hp}/${player.maxHp}${oldHp > player.hp ? ` (-${oldHp - player.hp})` : ""}`,
        "info",
    );
    ...
    if (player.hp <= 0) {
        ...
        displayMessage(
            "You lose consciousness and wake up in a safe place...",
            "info",
        );
        ...
        displayMessage(
            `💰 You lost ${goldLoss} gold from your ordeal.`,
            "error",
        );
        displayMessage(
            `❤️ You recover to ${player.hp} HP at a safe location.`,
            "info",
        );
        ...
        if (player.currentLocation !== randomSafeLocation) {
            ...
            displayMessage(
                `🏠 You find yourself at ${randomSafeLocation}.`,
                "info",
            );
        }
        ...
        addToConversationHistory(
            "assistant",
            `${player.name} was defeated in combat and recovered at ${player.currentLocation}`,
        );
    } else {
...
function initializePartySystem() {
    if (typeof PartyManager !== "undefined") {
        ...
        console.warn(
            "PartyManager class not found - party features will be limited",
        );
    }
}
...
async function recruitNPC(npcName, npcData = null) {
    ...
    if (!npc) {
        displayMessage(
            `You haven't met ${npcName} before, but you approach them to join your cause.`,
            "info",
        );
    ...
    // If we found an existing NPC, check if they are missing stats.
        ...
        console.log(
            `Existing NPC ${npc.name} found, but is missing stats. Generating now.`,
        );
    ...
    if (result.success) {
        updateRelationship(
            ...
            0,
            20,
            `${npcName} has joined your party as a trusted companion.`,
        );
...
function dismissPartyMember(memberId) {
    ...
    if (
        !confirm(
            `Are you sure you want to dismiss ${member.name} from your party?`,
        )
    ) {
        return;
    }
...
function displayPartyStatus() {
    ...
    allMembers.forEach((member) => {
        ...
        const loyaltyInfo = member.loyalty
            ? ` - Loyalty: ${member.loyalty}%`
            : "";

        displayMessage(
            `${member.name} ${status} - Level ${member.level} - ${healthStatus}${loyaltyInfo}`,
            "info",
        );
    });
    ...
    if (
        ...
        displayMessage(
            `Party Bonuses: +${bonuses.attack} Attack, +${bonuses.defense} Defense`,
            "success",
        );
        if (bonuses.special.length > 0) {
            displayMessage(
                `Special Abilities: ${bonuses.special.join(", ")}`,
                "success",
            );
        }
    }
...
async function initiateMultiCombat(enemies) {
    if (!multiCombatSystem || !partyManager) {
        ...
        return await CombatSystem.initiateCombat(
            player,
            enemies[0],
            player.currentLocation,
        );
    ...
    if (!combatResult.success) {
        displayMessage(
            "Failed to start multi-combat. Falling back to regular combat.",
            "error",
        );
        return await CombatSystem.initiateCombat(
            player,
...
function displayCombatState() {
    ...
    displayMessage(
        `Current Turn: ${state.currentTurn ? state.currentTurn.name : "None"}`,
        "info",
    );
    ...
    if (state.livingAllies.length > 0) {
        ...
        state.livingAllies.forEach((ally) => {
            displayMessage(
                `  ${ally.name}: ${ally.health}/${ally.maxHealth} HP`,
                "info",
            );
        });
    }
...
async function processCombatTurns() {
    while (multiCombatSystem.isActive) {
        ...
        if (currentTurn.type === "player") {
            ...
            displayMessage(
                `Available actions: Attack, Defend, Use Ability, Flee`,
                "info",
            );
...
function handleCombatEnd(result) {
    ...
    if (result.result === "victory") {
        ...
        displayMessage(
            `You gained ${playerXP} XP and ${baseGold} gold!`,
            "success",
        );
        F;
        // Check for level up
        ...
        displayMessage(
            "Party members recover slightly from the victory.",
            "success",
        );
        ...
        displayMessage(
            "Your party has been defeated but manages to escape...",
            "info",
        );
...
async function processItemChanges(itemChanges, player) {
    console.log(
        ...
        JSON.stringify(itemChanges, null, 2),
    );
    ...
    if (
        itemChanges.remove &&
        Array.isArray(itemChanges.remove) &&
        itemChanges.remove.length > 0
    ) {
        for (const itemToRemove of itemChanges.remove) {
            ...
            if (itemIndex !== -1) {
                ...
                console.log(
                    `processItemChanges: Removed item: ${removedItem.name}`,
                );
            } else {
                console.log(
                    `processItemChanges: Could not find item to remove: ${itemName}`,
                );
            }
        }
    }
    ...
    if (
        itemChanges.add &&
        Array.isArray(itemChanges.add) &&
        itemChanges.add.length > 0
    ) {
        for (const itemToAdd of itemChanges.add) {
            ...
            if (typeof itemToAdd === "string") {
                ...
                if (
                    window.ItemGenerator &&
                    typeof ItemGenerator.enhanceItem === "function"
                ) {
                    newItem = ItemGenerator.enhanceItem(itemToAdd, {
                        playerLevel: player.level,
                    });
                } else {
                    ...
                    newItem = {
                        ...
                        description:
                            ...
                            `A ${itemToAdd.name.toLowerCase()} found during your adventures.`,
                ...
                console.error(
                    "processItemChanges: Invalid item data:",
                    itemToAdd,
                );
                continue;
            }
            ...
            if (newItem && newItem.name && newItem.id) {
                ...
                if (!existingItem) {
                    ...
                    if (
                        window.ItemManager &&
                        typeof ItemManager.addItemToInventory === "function"
                    ) {
                        const success = ItemManager.addItemToInventory(
                            player,
                            newItem,
                        );
                        if (success) {
                            displayMessage(
                                `You obtained: ${newItem.name}!`,
                                "success",
                            );
                            console.log(
                                `processItemChanges: Added item: ${newItem.name}`,
                            );
                        }
                    } else {
                        if (!player.inventory) player.inventory = [];
                    ...
                    console.log(
                        `processItemChanges: Item ${newItem.name} already exists, skipping duplicate`,
                    );
                }
            } else {
                console.error(
                    `processItemChanges: Generated item is invalid:`,
                    newItem,
                );
            }
        }
    ...
    if (
        inventoryInterface &&
        !inventoryInterface.classList.contains("hidden") &&
        typeof displayInventory === "function"
    ) {
        displayInventory();
    }
...
async function handleItemPickup(command, storyContext = "") {
    ...
    if (itemsAdded > 0) {
        displayMessage(
            `${itemsAdded} item(s) added to inventory. You now have ${player.inventory.length} items total.`,
            "info",
        );
...
function extractItemsFromText(command, storyContext = "") {
    ...
    const storyItemMatches = storyContext.match(
        /(?:bronze helmet|short sword|leather jerkin|parchment|sword|helmet|armor|weapon|blade|axe|bow|staff|ring|amulet|potion|scroll|book|gem|crystal|equipment)/gi,
    );
    if (storyItemMatches) {
    ...
    const filteredItems = [...new Set(items)].filter(
        (item) =>
            ![
                "the",
                "and",
...
async function generateItemFromDescription(itemName, context) {
    try {
        ...
        if (generatedItem) {
            ...
            if (!generatedItem.id)
                generatedItem.id = ItemGenerator.generateItemId();
            if (!generatedItem.value)
                generatedItem.value = Math.max(
                    1,
                    Math.floor(Math.random() * 50) + 10,
                );
            if (!generatedItem.type)
                generatedItem.type = itemData.category || "trinket";
            if (!generatedItem.rarity)
                generatedItem.rarity = itemData.rarity || "COMMON";
        ...
        console.warn(
            `Failed to generate item for "${cleanName}", creating fallback item`,
        );
...
function createFallbackItem(itemName, itemData = {}) {
    ...
    return {
        id: ItemGenerator
            ? ItemGenerator.generateItemId()
            : `fallback_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
...
async function categorizeItemFromName(itemName) {
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            if (
                categories.includes(parsedJson.category) &&
                rarities.includes(parsedJson.rarity)
            ) {
        ...
        console.error(
            `AI categorization failed for "${itemName}". Error:`,
            error,
        );
...
WORLD CONTEXT:
- Current NPCs in area: ${
        getNPCsInLocation(player.currentLocation)
            .map((npc) => npc.name)
            .join(", ") || "None"
    }
- Player relationships: ${
        Object.keys(player.relationships || {})
            .slice(0, 3)
    ...
    try {
        ...
        const enhancedQuest = {
            ...
            rewards: {
                gold: Math.max(
                    0,
                    parseInt(geminiQuest.rewards?.gold) ||
                        questData.rewards.gold,
                ),
                experience: Math.max(
                    0,
                    parseInt(geminiQuest.rewards?.experience) ||
                        questData.rewards.experience,
                ),
                items: Array.isArray(geminiQuest.rewards?.items)
                    ? geminiQuest.rewards.items
                    : questData.rewards.items,
            },
            requirements: Array.isArray(geminiQuest.requirements)
                ? geminiQuest.requirements
        ...
        addToConversationHistory(
            "assistant",
            `New quest generated: ${enhancedQuest.title} - ${enhancedQuest.description}`,
        );
...
function displayLevelUpRewards(progression, oldLevel) {
    ...
    if (levelData) {
        if (levelData.features.length > 0) {
            displayMessage(
                `New Features: ${levelData.features.join(", ")}`,
                "success",
            );
        }
        if (levelData.abilities.length > 0) {
            displayMessage(
                `New Abilities: ${levelData.abilities.join(", ")}`,
                "success",
            );
        }
        if (levelData.spells.length > 0) {
            displayMessage(
                `New Spells Available: ${levelData.spells.join(", ")}`,
                "success",
            );
        }
        if (levelData.cantrips.length > 0) {
            displayMessage(
                `New Cantrips: ${levelData.cantrips.join(", ")}`,
                "success",
            );
        }
        if (levelData.feats.length > 0) {
            displayMessage(
                `New Feats: ${levelData.feats.join(", ")}`,
                "success",
            );
        }
    }
...
function displayQuests() {
    ...
    if (!player.quests || player.quests.length === 0) {
        ...
        if (completedQuests.length > 0) {
            const startIndex =
                questPagination.completedQuestsPage *
                questPagination.itemsPerPage;
            const endIndex = Math.min(
                startIndex + questPagination.itemsPerPage,
                completedQuests.length,
            );
            const paginatedQuests = completedQuests.slice(startIndex, endIndex);
            const totalPages = Math.ceil(
                completedQuests.length / questPagination.itemsPerPage,
            );
...
function displayCharacterProgression() {
    ...
    if (!progressionInterface) {
        displayMessage(
            "Error: Could not find progression interface element.",
            "error",
        );
        return;
    }
    ...
    if (!progressionContent) {
        displayMessage(
            "Error: Could not find progression content element.",
            "error",
        );
        return;
    }
    ...
    const partyStatusContainer = document.getElementById(
        "party-manager-display",
    );
    ...
    setTimeout(() => {
        if (prayerEffects.length > 0) {
            ...
            if (effect.healAmount) {
                ...
                player.hp = Math.min(
                    player.maxHp,
                    player.hp + effect.healAmount,
                );
                const actualHeal = player.hp - oldHp;
                if (actualHeal > 0) {
                    displayMessage(
                        `${effect.name}: You recover ${actualHeal} HP!`,
                        "success",
                    );
                }
            }
            ...
            if (effect.goldBonus) {
                ...
                displayMessage(
                    `Fortune smiles upon you! You found ${effect.goldBonus} gold!`,
                    "success",
                );
            }
            ...
            if (effect.statBonus) {
                ...
                setTimeout(() => {
                    Object.entries(effect.statBonus).forEach(
                        ([stat, bonus]) => {
                            if (player.stats[stat] !== undefined) {
                                player.stats[stat] -= bonus;
                            }
                    ...
                    displayMessage(
                        `The ${effect.name.toLowerCase()} effect has worn off.`,
                        "info",
                    );
                    updatePlayerStatsDisplay();
            ...
            if (effect.effects && player.statusEffects) {
                effect.effects.forEach((statusEffect) => {
                    ...
                    player.statusEffects.push({
                        ...
                        type:
                            alignmentInfo.type.includes("evil") ||
                            alignmentInfo.type === "malevolent"
                                ? "negative"
                                : "positive",
                    });
                });
            ...
            displayMessage(
                `Effect duration: ${Math.floor(effect.duration / 60)} minutes`,
                "info",
            );
...
function initializeBGMSystem() {
    if (typeof BGMManager !== "undefined") {
        ...
        console.warn(
            "BGMManager class not found - BGM features will be limited",
        );
    }
}

function loadBGMSettings() {
    const bgmSettings = JSON.parse(
        localStorage.getItem("bgmSettings") ||
...
function setupBGMEventListeners() {
    ...
    const generateMusicBtn = document.getElementById(
        "generate-location-music-btn",
    );
    if (generateMusicBtn) {
...
async function generateLocationMusic() {
    ...
    try {
        displayMessage(
            "🎵 Generating background music for " +
                player.currentLocation +
                "...",
            "info",
        );
        ...
        const gameState = {
            ...
            questActive: player.quests
                ? player.quests.some((q) => !q.completed)
                : false,
            recentEvents: [],
        };

        const musicData = await bgmManager.generateLocationMusic(
            player,
            gameState,
        );
        ...
        if (musicData) {
            ...
            updateBGMStatus();
            displayMessage(
                "🎶 Background music generated and ready!",
                "success",
            );
        } else {
        ...
        displayMessage(
            "Error generating background music: " + error.message,
            "error",
        );
    }
}
...
async function explore(command) {
    // 'command' parameter is already being passed
...
`;
    ...
    const explorationResult = await callGeminiAPI(
        explorePrompt,
        0.7,
        1048,
        false,
    ); // Increased tokens slightly for structured JSON

    if (explorationResult) {
        let parsedResult;
        try {
            // Attempt to parse the AI's structured JSON response
            const cleanResponse = explorationResult
                .replace(/```json\n?/g, "")
                .replace(/```\n?/g, "")
                .trim();
            const jsonMatch = cleanResponse.match(/\{[\s\S]*\}/);
            if (jsonMatch) {
                ...
                throw new Error(
                    "No valid JSON structure found in AI response for explore.",
                );
            }

            // Basic validation of parsed structure
            if (
                !parsedResult.discovery ||
                !parsedResult.interactable ||
                !Array.isArray(parsedResult.interactable.people) ||
                !Array.isArray(parsedResult.interactable.objects)
            ) {
                throw new Error(
                    "Invalid structure for 'interactable' object in AI response.",
                );
            }
        } catch (parseError) {
            console.error(
                "Explore: Failed to parse structured JSON from AI. Falling back to simple parsing. Error:",
                parseError,
                "Raw response:",
                explorationResult,
            );
            // Fallback to old, less reliable parsing if AI doesn't give perfect JSON
            const discoveryMatch = explorationResult.match(
                /DISCOVERY:\s*(.+?)(?=\nINTERACTABLE:|$)/s,
            );
            const interactableMatch = explorationResult.match(
                /INTERACTABLE:\s*(.+?)$/s,
            );
            const discoveryText = discoveryMatch
                ? discoveryMatch[1].trim()
                : explorationResult;
            const interactablesFlat = interactableMatch
                ? interactableMatch[1].split(",").map((s) => s.trim())
                : [];

            parsedResult = {
                discovery: discoveryText,
                interactable: {
                    people: interactablesFlat.filter(
                        (e) => e.match(/^[A-Z]/) || e.includes(" "),
                    ), // Simple heuristic for people
                    objects: interactablesFlat.filter(
                        (e) => !(e.match(/^[A-Z]/) || e.includes(" ")),
                    ), // Everything else is object
                },
            };
            displayMessage(
                "Warning: AI returned unexpected format for 'explore'. Using fallback interpretation.",
                "error",
            );
        }
        ...
        for (const npcNameRaw of parsedResult.interactable.people) {
            ...
            const identity = await RelationshipMiddleware.resolveNpcIdentity(
                npcNameRaw,
                playerRelationshipsCopy,
                command,
                parsedResult.discovery,
            );
            console.log(
                `Explore: Resolved identity for "${npcNameRaw}": canonicalName="${identity.canonicalName}"`,
            );
            ...
            if (!npcAlreadyInLocation) {
                ...
                console.log(
                    `Explore: Created and saved new NPC to gameWorld.npcs: "${newNPC.name}" at "${newNPC.location}".`,
                );
            } else {
                console.log(
                    `Explore: NPC "${identity.canonicalName}" already exists in gameWorld.npcs for this location.`,
                );
                const existingNpcRef = existingNPCsInLocation.find(
                    (npc) => npc.name === identity.canonicalName,
                );
                if (
                    existingNpcRef &&
                    ...
                    existingNpcRef.description = identity.description;
                    console.log(
                        `Explore: Updated description for existing NPC "${existingNpcRef.name}".`,
                    );
                }
            }
            ...
            updateRelationship(
                ...
                identity.description,
                true,
            ); // No trust change, just ensure presence
            console.log(
                `Explore: Ensured relationship for "${identity.canonicalName}" in player.relationships.`,
            );
        }
        ...
        for (const objectNameRaw of parsedResult.interactable.objects) {
            ...
            console.log(
                `Explore: Identified interactable object: "${objectNameRaw}"`,
            );
        }
        ...
        if (interactablesForDisplay.length > 0) {
            displayMessage(
                `You can interact with: ${interactablesForDisplay.join(", ")}`,
                "info",
            );
        }
    } else {
        displayMessage(
            "The area seems quiet. You don't find anything of interest.",
            "info",
        );
    }
}
...
async function handleSpecificNPCInteraction(npcNameInput) {
    ...
    const existingNPCs = getNPCsInLocation(player.currentLocation);
    console.log(
        `handleSpecificNPCInteraction: Searching for "${npcNameInput}" in current location's NPCs:`,
        existingNPCs.map((n) => n.name),
    );

    const targetNPC = existingNPCs.find(
        (npc) => npc.name.toLowerCase() === npcNameInput.toLowerCase(),
    );

    if (targetNPC) {
        console.log(
            `handleSpecificNPCInteraction: Found NPC: "${targetNPC.name}"`,
        );
        ...
        if (response) {
            ...
            displayMessage(
                `${targetNPC.name} doesn't seem to have much to say right now.`,
                "info",
            );
        }
    } else {
        console.log(
            `handleSpecificNPCInteraction: NPC "${npcNameInput}" not found in gameWorld.npcs for this location.`,
        );
        displayMessage(
            `You look around, but you don't see anyone named "${npcNameInput}" here.`,
            "error",
        );
    }
}
...
async function showShop() {
    ...
    if (!shopInterface) {
        displayMessage(
            "Error: The main shop interface (ID 'shop-interface') was not found in the HTML. Cannot open shop.",
            "error",
        );
        console.error(
            "Shop interface element with ID 'shop-interface' not found or is null.",
        );
    ...
    if (!shopItems) {
        displayMessage(
            "Error: The shop items display area (ID 'shop-items') was not found within the shop interface. Cannot display shop items.",
            "error",
        );
        console.error(
            "Shop items display element with ID 'shop-items' not found within shop interface.",
        );
    ...
    if (shopHeader) {
        ...
        console.warn(
            "Shop header (h4) element not found within shop interface.",
        );
    }
    ...
    */
    ...
    for (let i = 0; i < itemCount; i++) {
        ...
        try {
            ...
            console.error(
                `Error generating item ${i + 1} (falling back to generic item):`,
                error,
            );
            displayMessage(
                `A shop item failed to generate. Replacing with a generic item.`,
                "error",
            );
            // Fallback for this specific item if AI generation fails
            const fallbackItem = ItemGenerator.generateGenericItem(
                ItemGenerator.getRandomCategory(),
                ItemGenerator.getRandomRarityKey(),
            );
    ...
    shopInventory.forEach((item, index) => {
        ...
        const affordabilityClass = canAfford
            ? "text-green-600"
            : "text-red-600";
        const buttonDisabled = canAfford ? "" : "disabled";
        const buttonClass = canAfford
            ? "btn-parchment"
            : "btn-parchment opacity-50 cursor-not-allowed";
...
async function talkToNPC(npcName) {
    ...
    if (response) {
        ...
        displayMessage(
            `${npcName} doesn't seem interested in talking right now.`,
        );
    }
}
...
function addExplorationContext(discovery, interactableElements = []) {
    ...
    if (currentExplorationContext.discoveries.length > 5) {
        currentExplorationContext.discoveries =
            currentExplorationContext.discoveries.slice(-5);
    }
}
...
function getExplorationContextString() {
    ...
    const relationships =
        Object.values(player.relationships)
            .map((rel) => `${rel.name}: ${rel.status}`)
            .join(", ") || "none";

    let context =
        `Player is currently in ${playerLocation}. ` +
...
async function startConversation(suggestion = null) {
    ...
    if (npcInfo) {
        ...
        const fallbackMessage =
            "You don't see anyone interesting to talk to right now.";
...
async function generateCharacterBackground() {
    ...
    const gender = Array.from(charGenderRadios).find(
        (radio) => radio.checked,
    )?.value;
...
async function generateCharacterPortrait() {
    ...
    if (player && player.name) {
        ...
        charGender = Array.from(charGenderRadios).find(
            (radio) => radio.checked,
        )?.value;
    ...
    if (!charName || !charClass || !charGender) {
        alert(
            "Please fill in character name, class, and gender before generating a portrait.",
        );
        if (generateBtn) {
    ...
    if (player && player.portraitUrl && player.name === charName) {
        console.log(
            "ℹ️ Portrait already exists for current player:",
            player.portraitUrl,
        );
        displayMessage(
            "A portrait has already been generated for this character.",
            "info",
        );
    ...
    try {
        ...
        if (portraitUrl) {
            ...
            img.onload = function () {
                ...
                displayMessage(
                    "Character portrait generated and loaded successfully!",
                    "success",
                );
            ...
            img.onerror = function () {
                console.error(
                    "Failed to load portrait image from:",
                    portraitUrl,
                );
                portraitContainer.innerHTML = `
                    <div class="text-center py-4">
                ...
                `;
                displayMessage(
                    "Portrait generated but failed to load. Try refreshing the page.",
                    "error",
                );
        ...
        displayMessage(
            "Failed to generate portrait. Using placeholder image.",
            "error",
        );
...
async function tryMultipleImageServices(name, gender, charClass, background) {
    const services = [
        {
            name: "AI Novel API",
            generate: () =>
                generateAINovelPortrait(name, gender, charClass, background), // Try AI Novel first
        },
        {
    ...
    console.log(
        "🎨 Starting portrait generation with services:",
    ...
    for (const service of services) {
        try {
            ...
            if (result) {
                ...
                displayMessage(
                    `Portrait generated using ${service.name}`,
                    "success",
                );
                return result;
            }
...
async function generatePicsumPortrait(name) {
...
async function generateAINovelPortrait(name, gender, charClass, background) {
    // Accept parameters
    ...
    try {
        ...
        try {
            ...
            throw new Error(
                `Invalid JSON response from AI Novel: ${jsonError.message}`,
            );
        }
        ...
        if (!imageUrl) {
            console.error(
                "No image URL found in AI Novel response. Available fields:",
                Object.keys(result),
            );
            ...
            throw new Error(
                "AI Novel API did not return an image URL in any expected field",
            );
        }
        ...
        try {
            ...
            displayMessage(
                `AI Novel returned invalid URL: ${imageUrl}`,
                "error",
            );
        ...
        if (error.name === "AbortError") {
            ...
            const networkMsg =
                "Network error connecting to AI Novel service (CORS/firewall/DNS issue)";
            console.error("❌ AI Novel network error:", networkMsg);
            displayMessage(networkMsg, "error");
            displayMessage(
                "This could be a CORS policy, firewall, or DNS resolution issue",
                "error",
            );
...
async function generateSVGPortrait(name, charClass) {
    // Accept parameters
    const colors = {
        warrior: "#8B4513",
        mage: "#4B0082",
...
function createPlaceholderPortrait(name, charClass, level = 1) {
    // Accept parameters
    const colors = {
        warrior: "#8B4513",
        mage: "#4B0082",
...
function createMasterPrompt(command) {
    ...
    const playerState = {
        ...
        equipment: player.equipment,
        activeQuests:
            player.quests?.filter((q) => !q.completed).map((q) => q.title) ||
            [],
    };

    // Add multiplayer state if active
    const multiplayerState =
        isMultiplayerMode && multiplayerClient
            ? {
                  isMultiplayer: true,
                  isPlayerTurn: multiplayerClient.isMyTurn(),
                  isHost: multiplayerClient.isHost,
                  partyMembers: multiplayerClient.players.map((p) => ({
                      name: p.name,
                      id: p.id,
                  })),
                  currentTurnPlayer: multiplayerClient.players.find(
                      (p) => p.id === multiplayerClient.currentTurn,
                  )?.name,
              }
            : { isMultiplayer: false };
    ...
    const worldState = {
        ...
        nearbyNPCs: getNPCsInLocation(player.currentLocation).map(
            (npc) => npc.name,
        ),
        locationDescription:
            gameWorld.locationMemory.get(player.currentLocation) ||
            `A place known as ${player.currentLocation}.`,
...
async function executeCustomCommand(command) {
    ...
    for (const keyword of enhanceKeywords) {
        if (lowerCommand.includes(keyword)) {
            ...
            if (result && result.handled) {
                // Added check for result existence
                return;
            }
        }
    }
    ...
    for (const keyword of recruitKeywords) {
        if (lowerCommand.startsWith(keyword + " ")) {
            ...
            if (npcName) {
                ...
                const narrative = await callGeminiAPI(
                    narrativePrompt,
                    0.7,
                    300,
                    true,
    ...
    try {
        ...
        try {
            ...
            console.warn(
                "AI response contained malformed JSON. Treating as narrative.",
                e,
            );
            parsedResult = null;
        }
        ...
        if (parsedResult) {
            ...
            if (isMultiplayerMode) {
                multiplayerClient.sendAction(
                    command,
                    parsedResult.narrative || "performed an action",
                );
                ...
                setTimeout(() => {
                    if (multiplayerClient.isMyTurn()) {
                        ...
                        displayMessage(
                            "Your turn has ended automatically",
                            "info",
                        );
                    }
                }, 2000);
            ...
            if (parsedResult.alignmentChange) {
                console.log(
                    "Processing alignment change from AI response:",
                    parsedResult.alignmentChange,
                );
                const alignmentResult = AlignmentSystem.updateAlignment(
                    player,
                    parsedResult.alignmentChange,
                );
                if (alignmentResult.changed) {
                    displayMessage(
                        `Your alignment has shifted. You are now ${alignmentResult.newType}.`,
                        "info",
                    );
                }
            }
            ...
            if (
                window.TransactionMiddleware &&
                typeof TransactionMiddleware.detectTransaction === "function"
            ) {
                console.log(
                    "Checking for transactions in unstructured AI response...",
                );
                const transactionData =
                    await TransactionMiddleware.detectTransaction(
                        aiResponse,
                        command,
                        player,
                ...
                if (transactionData && transactionData.hasTransaction) {
                    console.log(
                        "Transaction detected in unstructured response:",
                        transactionData,
                    );
                    await TransactionMiddleware.processTransaction(
                        transactionData,
                        player,
                    );
                }
        ...
        displayMessage(
            "You attempt your action, but something goes wrong. The world seems to resist your will.",
            "error",
        );
    } finally {
...
function movePartyWithPlayer(oldLocation, newLocation) {
    if (partyManager && partyManager.party.length > 0) {
        ...
        partyManager.party.forEach((partyMember) => {
            const indexInOldLocation = updatedOldLocationNPCs.findIndex(
                (npc) => npc.id === partyMember.id,
            );
            let movedNpc;
            ...
            if (indexInOldLocation > -1) {
                ...
                updatedOldLocationNPCs = updatedOldLocationNPCs.filter(
                    (npc) => npc.id !== partyMember.id,
                );
            } else {
...
async function parseAndApplyStateChanges(result) {
    if (!result) {
        console.error(
            "parseAndApplyStateChanges called with null or undefined result.",
        );
        return;
    }
    ...
    if (result.narrative) {
        ...
        if (
            ...
            if (transactionData && transactionData.hasTransaction) {
                console.log(
                    "Transaction detected in structured response:",
                    transactionData,
                );
                await TransactionMiddleware.processTransaction(
                    transactionData,
    ...
    if (result.playerStateChanges) {
        ...
        if (
            typeof result.playerStateChanges.gold === "number" &&
            result.playerStateChanges.gold !== 0
        ) {
            updateGold(result.playerStateChanges.gold, "transaction");
        }
        if (
            typeof result.playerStateChanges.exp === "number" &&
            result.playerStateChanges.exp !== 0
        ) {
            gainExperience(result.playerStateChanges.exp);
        }
        if (
            typeof result.playerStateChanges.hp === "number" &&
            result.playerStateChanges.hp !== 0
        ) {
            const oldHp = player.hp;
            player.hp = Math.max(
                0,
                Math.min(
                    player.maxHp,
                    player.hp + result.playerStateChanges.hp,
                ),
            );
            ...
            if (actualChange > 0) {
                ...
                displayMessage(
                    `You take ${Math.abs(actualChange)} damage.`,
                    "error",
                );
            }
        }
        ...
        if (result.playerStateChanges.statusEffects) {
            ...
            if (effects.remove) {
                effects.remove.forEach((effectName) => {
                    const index = player.statusEffects.findIndex(
                        (e) => e.name === effectName,
                    );
                    if (index > -1) {
                        displayMessage(
                            `The effect '${player.statusEffects[index].name}' fades.`,
                            "info",
                        );
            ...
            if (effects.add) {
                effects.add.forEach((effectToAdd) => {
                    ...
                    displayMessage(
                        `You are now affected by: ${effectToAdd.name}.`,
                        "warning",
                    );
                });
            }
    ...
    if (result.alignmentChange) {
        ...
        if (alignmentResult.changed) {
            displayMessage(
                `Your alignment has shifted. You are now considered ${alignmentResult.newType}.`,
                "info",
            );
        }
    }
    ...
    if (
        result.factionReputationChange &&
        result.factionReputationChange.length > 0
    ) {
        ...
        result.factionReputationChange.forEach((change) => {
            ...
            displayMessage(
                `Your reputation with ${change.faction} has ${trend}.`,
                "info",
            );
        });
    }
    ...
    if (result.itemChanges) {
        if (result.itemChanges.remove && result.itemChanges.remove.length > 0) {
            result.itemChanges.remove.forEach((itemToRemove) => {
                ...
                if (itemIndex > -1) {
                    ...
                    displayMessage(
                        `${itemToRemove.name} removed from inventory.`,
                        "info",
                    );
                }
            });
        ...
        if (result.itemChanges.add && result.itemChanges.add.length > 0) {
            result.itemChanges.add.forEach((itemToAdd) => {
                const newItem = ItemGenerator.enhanceItem(itemToAdd, {
                    playerLevel: player.level,
                });
    ...
    if (
        ...
        result.locationChange !== player.currentLocation
    ) {
        const oldLocation = player.currentLocation;
    ...
    if (result.questProgress) {
        const quest = player.quests.find(
            (q) => q.title === result.questProgress.questTitle && !q.completed,
        );
        if (quest) {
            if (result.questProgress.update) {
                displayMessage(
                    `Quest Update: ${result.questProgress.update}`,
                    "success",
                );
            }
...
function changeCompletedQuestsPage(direction) {
    const completedQuests = player.quests
        ? player.quests.filter((q) => q.completed)
        : [];
    const totalPages = Math.ceil(
        completedQuests.length / questPagination.itemsPerPage,
    );
    ...
    questPagination.completedQuestsPage = Math.max(
        0,
        Math.min(questPagination.completedQuestsPage, totalPages - 1),
    );
...
function changeInventoryPage(direction) {
    const unequippedItems = player.inventory
    ...
    const totalPages = Math.ceil(
        unequippedItems.length / inventoryPagination.itemsPerPage,
    );

    inventoryPagination.currentPage += direction;
    inventoryPagination.currentPage = Math.max(
        0,
        Math.min(inventoryPagination.currentPage, totalPages - 1),
    );
...
function buyShopItem(itemIndex) {
    // Renamed parameter to itemIndex for clarity and consistency
    if (
        !window.currentShopInventory ||
        !window.currentShopInventory[itemIndex]
    ) {
    ...
    if (player.gold < item.value) {
        displayMessage(
            `You need ${item.value} gold but only have ${player.gold} gold.`,
            "error",
        );
        return;
    }
    ...
    if (
        ...
        displayMessage(
            `Purchased ${item.name} for ${item.value} gold! (Fallback)`,
            "success",
        );
        if (
            typeof ItemManager !== "undefined" &&
    ...
    const itemElement = document.querySelector(
        `.shop-item:nth-child(${itemIndex + 1})`,
    );
    if (itemElement) {
        itemElement.remove();
    }
...
function startNewGame(player) {
    ...
    const firstGenderRadio = document.querySelector(
        'input[name="char-gender"]',
    );
...
function initializeMultiplayerUI() {
    ...
    const actionButtons = document.querySelector(
        "#game-play-screen .mb-4.flex.flex-wrap.gap-2",
    );
    if (actionButtons) {
...
function handleTurnChanged(data) {
    ...
    if (multiplayerClient.isMyTurn()) {
        ...
        const currentPlayer = multiplayerClient.players.find(
            (p) => p.id === data.currentTurn,
        );
        displayMessage(
            `It's ${currentPlayer?.name || "someone"}'s turn`,
            "info",
        );
    }
}
...
function handlePlayerAction(data) {
    const player = multiplayerClient.players.find(
...
function updatePlayerList(players) {
    ...
    players.forEach((player) => {
        ...
        playerDiv.className =
            "player-item p-2 mb-2 bg-amber-900/20 rounded border border-amber-700/30";
        playerDiv.innerHTML = `
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function abandonQuest(questId) {
        ...
        if (
            ...
            if (
                questInterface &&
                !questInterface.classList.contains("hidden")
            ) {
                displayQuests();
            }
            ...
            addToConversationHistory(
                "assistant",
                `${player.name} abandoned the quest: ${quest.title}`,
            );
            saveConversationHistory();
        }
    }
    ...
    window.LocationManager =
        typeof LocationManager !== "undefined" ? LocationManager : {};
    ...
    window.debugInventory =
        typeof debugInventory !== "undefined"
            ? debugInventory
            : function () {
                  /* ... */
              };
    window.fixInventory =
        typeof fixInventory !== "undefined"
            ? fixInventory
            : function () {
                  /* ... */
              };
    window.resetCharacterProgression =
        typeof resetCharacterProgression !== "undefined"
    ...
    if (inventoryItemsDisplay) {
        inventoryItemsDisplay.addEventListener("click", function (event) {
            ...
            if (button) {
                ...
                console.log(
                    `Inventory action: ${action}, Index: ${itemIndexStr}, Slot: ${slot}`,
                );

                // Ensure itemIndex is a valid number when needed
                const itemIndex =
                    itemIndexStr !== undefined
                        ? parseInt(itemIndexStr, 10)
                        : null;

                if (action !== "unequip" && itemIndex === null) {
                    console.error(
                        "Action requires an item index, but it's missing or invalid.",
                    );
                    displayMessage(
                        "Error: Item index not found for action.",
                        "error",
                    );
                    return;
                }
                ...
                if (
                    ...
                    console.error(
                        `Invalid item index: ${itemIndex} for inventory of length ${player.inventory?.length || 0}`,
                    );
                    displayMessage(
                        "Error: Invalid item selected or inventory not ready.",
                        "error",
                    );
                    return;
                }
                ...
                switch (action) {
                    case "use":
                        if (itemIndex !== null)
                            ItemManager.useItem(player, itemIndex); // MODIFIED
                        break;
                    case "equip":
                        if (itemIndex !== null)
                            ItemManager.equipItem(player, itemIndex); // MODIFIED
                        break;
                    case "sell":
                        if (itemIndex !== null)
                            ItemManager.sellItem(player, itemIndex); // MODIFIED
                        break;
                    case "drop":
                        if (itemIndex !== null)
                            ItemManager.dropItem(player, itemIndex); // MODIFIED
                        break;
                    case "unequip": // 'unequip' uses 'data-slot'
                        if (slot) {
                            ...
                            console.error(
                                "Unequip action requires a slot, but it's missing.",
                            );
                            displayMessage(
                                "Error: Slot not found for unequip action.",
                                "error",
                            );
                        }
                        break;
    ...
    if (shopItemsDisplay) {
        shopItemsDisplay.addEventListener("click", function (event) {
            ...
            if (button) {
                ...
                if (itemIndex === null) {
                    console.error(
                        "Shop action requires an item index, but it's missing or invalid.",
                    );
                    displayMessage(
                        "Error: Item index not found for shop action.",
                        "error",
                    );
                    return;
                }
                ...
                if (
                    !window.currentShopInventory ||
                    itemIndex < 0 ||
                    itemIndex >= window.currentShopInventory.length
                ) {
                    console.error(
                        `Invalid item index: ${itemIndex} for shop inventory of length ${window.currentShopInventory?.length || 0}`,
                    );
                    displayMessage(
                        "Error: Invalid item selected or shop inventory not ready.",
                        "error",
                    );
                    return;
                }
    ...
    const illustrationToggle = document.getElementById(
        ...
        illustrationToggle.addEventListener("click", async () => {
            ...
            if (isIllustrationModeActive) {
                if (
                    ...
                    illustrationToggle.innerHTML =
                        '🖼️<span class="hidden sm:inline"> Illustration Mode: ON</span><span class="sm:hidden"> ON</span>';
                    illustrationToggle.className =
                        "btn-parchment bg-green-600 hover:bg-green-700 text-white text-xs md:text-sm py-1 px-2";
                    ...
                    illustrationToggle.innerHTML =
                        '🖼️<span class="hidden sm:inline"> Illustration Mode</span><span class="sm:hidden"> Illus</span>';
                    illustrationToggle.className =
                        "btn-parchment bg-indigo-600 hover:bg-indigo-700 text-white text-xs md:text-sm py-1 px-2";
                }
            } else {
                illustrationToggle.innerHTML =
                    '🖼️<span class="hidden sm:inline"> Illustration Mode</span><span class="sm:hidden"> Illus</span>';
            ...
            if (isIllustrationModeActive) {
                if (
                    ...
                    if (illustrationModeBtn) {
                        ...
                        illustrationModeBtn.classList.remove(
                            "btn-active-style",
                        );
                    }
                }
    ...
    const sceneryImageDisplay = document.getElementById(
        "scenery-image-display",
    );
    const closeSceneryImageBtn = document.getElementById(
        "close-scenery-image-btn",
    );
    ...
    if (sceneryImageDisplay) {
        sceneryImageDisplay.addEventListener("click", (event) => {
            if (event.target === sceneryImageDisplay) {
...
removePortraitBtn.className =
    "btn-parchment bg-red-600 hover:bg-red-700 text-white text-xs md:text-sm py-1 px-2";
removePortraitBtn.style.cssText = `
    font-size: 0.75rem;
`;
removePortraitBtn.innerHTML =
    '<i class="ra ra-cancel mr-1"></i><span class="hidden sm:inline">Remove Portrait</span><span class="sm:hidden">Del Pic</span>';
...
resetProgressionBtn.className =
    "btn-parchment bg-orange-600 hover:bg-orange-700 text-white text-xs md:text-sm py-1 px-2";
resetProgressionBtn.style.cssText = `
    font-size: 0.75rem;
`;
resetProgressionBtn.innerHTML =
    '<i class="ra ra-recycle mr-1"></i><span class="hidden sm:inline">Reset Progression</span><span class="sm:hidden">Reset</span>';
resetProgressionBtn.title =
    "Reset character progression (feats, skills, abilities) to match updated game files";
...
richTextToggle.className =
    "btn-parchment rich-text-toggle bg-purple-600 hover:bg-purple-700 text-white text-xs md:text-sm py-1 px-2";
richTextToggle.style.cssText = `
    font-size: 0.75rem;
`;
richTextToggle.innerHTML =
    '<i class="ra ra-fireball mr-1"></i><span class="hidden sm:inline">Rich Text: OFF</span><span class="sm:hidden">RT: OFF</span>';
...
function toggleRichText() {
    ...
    displayMessage(
        `Rich text styling ${richTextEnabled ? "enabled" : "disabled"}.`,
        "info",
    );
}
...
function processRichText(text, messageType = null) {
    ...
    processed = processed.replace(
        /\{\{underline:(.*?)\}\}/g,
        '<span class="rt-underline">$1</span>',
    );

    // Strikethrough: ~~text~~
    processed = processed.replace(
        /~~(.*?)~~/g,
        '<span class="rt-strikethrough">$1</span>',
    );
    ...
    processed = processed.replace(
        /\{red:(.*?)\}/g,
        '<span class="rt-color-red">$1</span>',
    );
    processed = processed.replace(
        /\{green:(.*?)\}/g,
        '<span class="rt-color-green">$1</span>',
    );
    processed = processed.replace(
        /\{blue:(.*?)\}/g,
        '<span class="rt-color-blue">$1</span>',
    );
    processed = processed.replace(
        /\{purple:(.*?)\}/g,
        '<span class="rt-color-purple">$1</span>',
    );
    processed = processed.replace(
        /\{gold:(.*?)\}/g,
        '<span class="rt-color-gold">$1</span>',
    );
    processed = processed.replace(
        /\{silver:(.*?)\}/g,
        '<span class="rt-color-silver">$1</span>',
    );
    processed = processed.replace(
        /\{crimson:(.*?)\}/g,
        '<span class="rt-color-crimson">$1</span>',
    );
    processed = processed.replace(
        /\{emerald:(.*?)\}/g,
        '<span class="rt-color-emerald">$1</span>',
    );
    processed = processed.replace(
        /\{brown:(.*?)\}/g,
        '<span class="rt-color-brown">$1</span>',
    );

    // Shadow effects: {shadow:text}
    processed = processed.replace(
        /\{shadow:(.*?)\}/g,
        '<span class="rt-shadow">$1</span>',
    );

    // Handle contextual color words - map to appropriate colors
    processed = processed.replace(
        /\{magical\}/g,
        '<span class="rt-color-purple">magical</span>',
    );
    processed = processed.replace(
        /\{infernal energy\}/g,
        '<span class="rt-color-red">infernal energy</span>',
    );
    processed = processed.replace(
        /\{infernal\}/g,
        '<span class="rt-color-red">infernal</span>',
    );
    processed = processed.replace(
        /\{peaceful\}/g,
        '<span class="rt-color-green">peaceful</span>',
    );
    processed = processed.replace(
        /\{molten rock\}/g,
        '<span class="rt-color-crimson">molten rock</span>',
    );
    processed = processed.replace(
        /\{molten\}/g,
        '<span class="rt-color-crimson">molten</span>',
    );
    processed = processed.replace(
        /\{ash\}/g,
        '<span class="rt-color-silver">ash</span>',
    );
    processed = processed.replace(
        /\{sulfur\}/g,
        '<span class="rt-color-gold">sulfur</span>',
    );
    processed = processed.replace(
        /\{flames\}/g,
        '<span class="rt-color-red">flames</span>',
    );
    processed = processed.replace(
        /\{lava flows\}/g,
        '<span class="rt-color-crimson">lava flows</span>',
    );
    processed = processed.replace(
        /\{lava\}/g,
        '<span class="rt-color-crimson">lava</span>',
    );
    processed = processed.replace(
        /\{bridges\}/g,
        '<span class="rt-color-silver">bridges</span>',
    );
    processed = processed.replace(
        /\{epic\}/g,
        '<span class="rt-color-gold">epic</span>',
    );
    processed = processed.replace(
        /\{hellish\}/g,
        '<span class="rt-color-red">hellish</span>',
    );
    processed = processed.replace(
        /\{portal\}/g,
        '<span class="rt-color-blue">portal</span>',
    );
    ...
    processed = processed.replace(/\{([a-zA-Z\s]+)\}/g, function (match, word) {
        ...
        if (
            ...
            word.includes("magic") ||
            word.includes("arcane") ||
            word.includes("mystic")
        ) {
            return `<span class="rt-color-purple">${word}</span>`;
        } else if (
            word.includes("nature") ||
            word.includes("green") ||
    ...
    processed = processed.replace(
        /\[medieval:(.*?)\]/g,
        '<span class="rt-font-medieval">$1</span>',
    );
    processed = processed.replace(
        /\[magic:(.*?)\]/g,
        '<span class="rt-font-magic">$1</span>',
    );
    processed = processed.replace(
        /\[elegant:(.*?)\]/g,
        '<span class="rt-font-elegant">$1</span>',
    );
    processed = processed.replace(
        /\[ancient:(.*?)\]/g,
        '<span class="rt-font-ancient">$1</span>',
    );
    ...
    processed = processed.replace(
        /\{\{highlight:(.*?)\}\}/g,
        '<span class="rt-highlight">$1</span>',
    );
    processed = processed.replace(
        /\{\{blink:(.*?)\}\}/g,
        '<span class="rt-blink">$1</span>',
    );
    processed = processed.replace(
        /\{\{wiggle:(.*?)\}\}/g,
        '<span class="rt-wiggle">$1</span>',
    );
    processed = processed.replace(
        /\{\{shadow:(.*?)\}\}/g,
        '<span class="rt-shadow">$1</span>',
    ...
    processed = processed.replace(
        /\{\{glow:(.*?)\}\}/g,
        '<span class="rt-glow">$1</span>',
    );
    processed = processed.replace(
        /\{\{stretch-h:(.*?)\}\}/g,
        '<span class="rt-stretch-h">$1</span>',
    );
    processed = processed.replace(
        /\{\{stretch-v:(.*?)\}\}/g,
        '<span class="rt-stretch-v">$1</span>',
    );
    processed = processed.replace(
        /\{\{glow-shadow:(.*?)\}\}/g,
        '<span class="rt-shadow-glow">$1</span>',
    );
    processed = processed.replace(
        /\{\{underline:(.*?)\}\}/g,
        '<span class="rt-underline">$1</span>',
    ...
    try {
        ...
        newGameBtn?.addEventListener("click", () => {
            if (!gameSettings.apiKey || !gameSettings.model) {
                alert(
                    "A Gemini API Key and Model are required. Please configure them in the Settings menu.",
                );
                document
        ...
        // --- Character Creation ---
        generateBackgroundBtn?.addEventListener(
            "click",
        ...
        document.getElementById("rest-btn")?.addEventListener("click", () => {
            ...
            const restMessage = `You rest and recover ${healAmount} HP. You feel refreshed.`;
            displayMessage(restMessage, "success");
            updatePlayerStatsDisplay();

            if (isMultiplayerMode) {
                multiplayerClient.sendAction("rested", restMessage);
                multiplayerClient.endTurn();
            }
            saveGame();
        });
        ...
        document
            ...
            ?.addEventListener("click", () => {
                ...
                // explore() is async, so we wrap this to send the action *after* it's done
                explore("explore the area").then(() => {
                    if (isMultiplayerMode) {
                        multiplayerClient.sendAction(
                            "explored the area",
        ...
        document
            .getElementById("cast-spell-btn")
            ?.addEventListener("click", () => {
                if (isMultiplayerMode && !multiplayerClient.isMyTurn()) {
                    displayMessage("Wait for your turn to act.", "error");
                    return;
                }
                displaySkillsAndAbilities();
                // Note: The actual action is sent when a spell/ability is selected inside displaySkillsAndAbilities
            });
        ...
        document.getElementById("pray-btn")?.addEventListener("click", () => {
            ...
            if (isMultiplayerMode) {
                multiplayerClient.sendAction(
                    "prayed",
                    "A prayer was offered to the gods.",
                );
                multiplayerClient.endTurn();
            }
        });
        document
            ...
            ?.addEventListener("click", () =>
                displayMessage(HelpSystem.getHelp(), "info"),
            );
        ...
        showBackgroundBtn?.addEventListener(
            "click",
            displayCharacterBackground,
        );
        ...
        async function processItemChanges(itemChanges, player) {
            ...
            if (
                itemChanges.itemsToAdd &&
                Array.isArray(itemChanges.itemsToAdd)
            ) {
                for (const itemData of itemChanges.itemsToAdd) {
                    ...
                    try {
                        ...
                        if (
                            window.ItemGenerator &&
                            typeof ItemGenerator.generateItem === "function"
                        ) {
                            const context = {
                                category:
                        ...
                        if (!newItem) {
                            newItem = {
                                ...
                                description:
                                    ...
                                    `A ${itemData.name.toLowerCase()} obtained during your adventure.`,
                                value:
                                    itemData.value ||
                                    Math.max(
                                        1,
                        ...
                        if (
                            ...
                            ItemManager.addItemToInventory(player, newItem);
                            displayMessage(
                                `Added to inventory: ${newItem.name}`,
                                "success",
                            );
                        } else {
                        ...
                        displayMessage(
                            `Failed to add item: ${itemData.name}`,
                            "error",
                        );
                    }
                }
            ...
            if (
                ...
                Array.isArray(itemChanges.itemsToRemove)
            ) {
                for (const itemName of itemChanges.itemsToRemove) {
                    ...
                    if (itemIndex !== -1) {
                        const removedItem = player.inventory.splice(
                            itemIndex,
                            1,
                        )[0];
                        displayMessage(
                            `Removed from inventory: ${removedItem.name}`,
                            "info",
                        );
                    }
                }
            ...
            if (
                itemChanges.goldChange &&
                typeof itemChanges.goldChange === "number" &&
                itemChanges.goldChange !== 0
            ) {
        ...
        document
            .getElementById("show-progression-btn")
            ?.addEventListener("click", displayCharacterProgression);
        ...
        document
            .getElementById("exit-progression-btn")
            ?.addEventListener("click", () =>
                document
                    .getElementById("progression-interface")
...
function updateQuickActionButtons() {
    ...
    switch (player.class.toLowerCase()) {
        ...
        case "ranger":
            ...
            castSpellBtn.title =
                "Use a ranger technique or nature-based ability";
            break;
        case "cleric":
...
function createCharacter() {
    ...
    player.background =
        charBackgroundTextarea.value || "A mysterious adventurer.";
    ...
    updatePlayerStatsDisplay();
    displayMessage(
        `Welcome to Pedena, ${player.name}! Your adventure begins in ${player.currentLocation}.`,
    );
    updateQuickActionButtons();
...
function displayInventory() {
    ...
    if (typeof ItemManager !== "undefined" && ItemManager.updateStatusEffects) {
        ...
        typeof window.ItemManager !== "undefined" &&
        window.ItemManager.updateStatusEffects
    ) {
        window.ItemManager.updateStatusEffects(player);
    }

    console.log(
        "displayInventory: Rendering inventory. Items count:",
        player.inventory ? player.inventory.length : "undefined",
    );
    ...
    if (player.statusEffects && player.statusEffects.length > 0) {
        ...
        player.statusEffects.forEach((effect) => {
            const timeLeft = Math.max(
                0,
                Math.floor(((effect.expiresAt || 0) - Date.now()) / 1000),
            );
    ...
    const startIndex =
        inventoryPagination.currentPage * inventoryPagination.itemsPerPage;
    const endIndex = Math.min(
        startIndex + inventoryPagination.itemsPerPage,
    ...
    if (unequippedItems.length === 0) {
        inventoryHTML +=
            '<p class="text-center text-gray-600">Your inventory is empty.</p>';
    } else {
        paginatedItems.forEach((item, index) => {
            // Find the original index in the full inventory for the action buttons
            const originalIndex = player.inventory.findIndex(
                (invItem) => invItem.id === item.id,
            );
            inventoryHTML +=
                typeof buildInventoryItemDisplay === "function"
                    ? buildInventoryItemDisplay(item, originalIndex)
                    : ``;
        });
    }
    inventoryHTML += `
    ...
    if (inventoryItemsDisplay) {
        ...
        console.error(
            "inventoryItemsDisplay element not found in displayInventory!",
        );
    }
}
...
function getIconForItem(item) {
    ...
    switch (inferredType) {
        ...
        case "armor":
            switch (item.slot) {
                case "head":
                    return "ra-knight-helmet";
                case "chest":
                    return "ra-vest";
                case "hands":
                    return "ra-gauntlet";
                case "legs":
                    return "👖"; // FIX: Use emoji for equipped leg items
                case "feet":
                    return "ra-boots";
                case "back":
                    return "🧥"; // FIX: Use emoji for equipped back items
                case "offHand":
                    return "ra-shield";
                default:
                    return "ra-armor";
        ...
        .map((slotData) => {
            ...
            if (item) {
                ...
                function handleDonation(amount) {
                    ...
                    displayMessage(
                        `Thank you for your ${amount} donation! Opening Venmo payment...`,
                        "success",
                    );
                ...
                document.addEventListener("DOMContentLoaded", function () {
                    ...
                    if (donationModalCloseBtn) {
                        donationModalCloseBtn.addEventListener(
                            "click",
                            closeDonationModal,
                        );
                    }
                ...
                if (item.effects && Array.isArray(item.effects)) {
                    const combatEffects = item.effects.filter(
                        (effect) =>
                            effect.includes("strength") ||
                            effect.includes("dexterity") ||
                            effect.includes("constitution") ||
                            effect.includes("defense") ||
                            effect.includes("attack") ||
                            effect.includes("damage") ||
                            effect.includes("armor") ||
                            effect.includes("resist"),
                    );
...
function buildInventoryItemDisplay(item, index) {
    ...
    const isConsumable =
        ...
        (item.effect &&
            (item.effect.type === "heal" || item.effect.type === "mana"));
...
function getEffectDescription(effect) {
    ...
    if (effect.type === "buff") {
        if (
            ...
            statusEffects[effect.status]
        ) {
            // This is a named status effect like "swift"
            const statusDef = statusEffects[effect.status];
            let description =
                statusDef.name ||
                effect.status.charAt(0).toUpperCase() + effect.status.slice(1); // Use defined name or capitalize status
            if (effect.duration) {
    ...
    Object.values(player.equipment).forEach((item) => {
        ...
        if (item.effects && Array.isArray(item.effects)) {
            item.effects.forEach((effect) => {
                ...
                if (effect.includes("intelligence_boost"))
                    bonuses.intelligence += 1;
                if (effect.includes("constitution_boost"))
...
function equipItem(itemIndex) {
    ...
    if (!player.inventory || !player.inventory[itemIndex]) {
        displayMessage(
            "Item not found in inventory at index: " + itemIndex,
            "error",
        );
        console.error(
            "Item not found in inventory at index:",
            itemIndex,
            "Current inventory:",
    ...
    console.log(
        "Current player.equipment state:",
    ...
    if (!item.slot) {
        displayMessage(
            "This item cannot be equipped (no slot defined).",
            "error",
        );
        return;
    }
    ...
    if (
        ...
        // More specific check for rings
        console.log("Processing as a ring. Item slot:", item.slot);
        console.log(
            "ring1 currently:",
            player.equipment.ring1 ? player.equipment.ring1.name : "Empty",
        );
        console.log(
            "ring2 currently:",
            player.equipment.ring2 ? player.equipment.ring2.name : "Empty",
        );
        ...
        if (player.equipment.ring1 === null) {
            ...
            targetSlot = "ring1";
            displayMessage(
                "Both ring slots are full. Replacing the ring in the first slot.",
                "info",
            );
            console.log(
                "Both ring slots full. Targeting ring1 for replacement.",
            );
        }
    } else {
        console.log(
            `Item is not a ring or has a specific non-ring slot. Target slot: ${targetSlot}`,
        );
    }
    ...
    if (!player.equipment.hasOwnProperty(targetSlot)) {
        displayMessage(
            `Cannot equip: Invalid target slot '${targetSlot}' on player equipment object.`,
            "error",
        );
        console.error(
            `Invalid target slot: ${targetSlot}. Player equipment keys: ${Object.keys(player.equipment)}`,
        );
        return;
    }
    ...
    if (player.equipment[targetSlot] !== null) {
        ...
        displayMessage(
            `Unequipped ${oldItem.name} from ${targetSlot}.`,
            "info",
        );
    ...
    if (item.effects && item.effects.length > 0) {
        // Check effects array has content
        if (
            typeof ItemManager !== "undefined" &&
            typeof ItemManager.applyItemEffects === "function"
        ) {
            ItemManager.applyItemEffects(player, item);
        } else if (
            typeof window.ItemManager !== "undefined" &&
            typeof window.ItemManager.applyItemEffects === "function"
        ) {
...
function useItem(itemIndex) {
    ...
    if (item.effect) {
        if (item.effect.type === "heal") {
            ...
            const actualHeal = player.hp - oldHp;
            displayMessage(
                `You used ${item.name} and recovered ${actualHeal} HP.`,
                "success",
            );
            effectApplied = true;
        } else if (item.effect.type === "mana") {
            displayMessage(
                `You used ${item.name} and feel your magical energy restored.`,
                "success",
            );
            effectApplied = true;
        } else if (
...
async function appraiseItem(itemIndex) {
    ...
    displayMessage(
        `You focus on the mysterious item, trying to discern its properties...`,
        "info",
    );
    ...
    try {
        ...
        if (newItem) {
            ...
            displayMessage(
                `The item's true nature is revealed! You have identified: <strong>${newItem.name}</strong>!`,
                "success",
            );
            ItemManager.saveInventoryToStorage(player);
        ...
        displayMessage(
            "You couldn't quite figure out what the item is. Perhaps try again later.",
            "error",
        );
    }
}
...
async function applyAbilityEffects(abilityName, abilityDef) {
    if (!abilityDef) {
        displayMessage(
            `The ability "${abilityName}" is not defined correctly.`,
            "error",
        );
        return;
    }
    ...
    if (abilityDef.effect?.damage && player.currentEnemy) {
        ...
        displayMessage(
            `Your ${abilityName} deals an extra ${damage} damage to ${player.currentEnemy.name}!`,
            "combat",
        );
    }
    if (abilityDef.effect?.condition && player.currentEnemy) {
        displayMessage(
            `${player.currentEnemy.name} is now ${abilityDef.effect.condition}!`,
            "combat",
        );
    }
...
async function useAbilityOrSpell(type, name) {
    ...
    if (type === "spell" || type === "cantrip") {
        ...
        if (!definition) {
            displayMessage(
                `Definition for spell "${name}" not found.`,
                "error",
            );
            return;
        }
        ...
        if (outcome) {
            ...
            displayMessage(
                `You cast ${name}, and arcane energy fills the air.`,
                "info",
            );
        }
        ...
        displayMessage(
            `You focus on your training in "${name}". (${definition.description})`,
            "info",
        );
...
function displaySkillsAndAbilities() {
    ...
    if (skillsHTML.trim() === "") {
        skillsHTML =
            '<p class="text-center text-gray-600">You have no special skills or abilities to use at this time.</p>';
    }
...
async function generateCharacterPortraitOld() {
    ...
    try {
        ...
        if (result.imageUrl) {
            ...
            displayMessage(
                "Character portrait generated successfully!",
                "success",
            );
            if (generateBtn) {
        ...
        displayMessage(
            "Failed to generate character portrait. Please try again later.",
            "error",
        );
        if (generateBtn) {
            generateBtn.disabled = false;
...
function displayCharacterBackground() {
    ...
    const backgroundContentDisplay =
    ...
    if (
        typeof calculateEquipmentBonuses === "function" &&
        typeof buildStatsGrid === "function"
    ) {
        ...
        console.warn(
            "Detailed stats functions (calculateEquipmentBonuses or buildStatsGrid) missing. Using basic stats display.",
        );
    ...
    if (player.relationships && Object.keys(player.relationships).length > 0) {
        ...
        if (sortedRelationships.length > 0) {
            ...
            sortedRelationships.forEach(([npcName, relationship]) => {
                ...
                const firstMeeting =
                    relationship.firstMeeting || "Some time ago";
                const description =
                    relationship.description || "A person you've encountered.";
            ...
            relationshipsHTML =
                '<p class="text-sm text-gray-600 italic">No valid relationships found.</p>';
        }
    } else {
        relationshipsHTML =
            '<p class="text-sm text-gray-600 italic">No relationships established yet.</p>';
    }
    ...
    player.inventory.forEach((item) => {
        if (!item.slot && item.type) {
            ...
            if (
                ...
                console.log(
                    `Skipping ${item.name}: type '${item.type}' should not have a slot`,
                );
                return;
            }
            ...
            if (item.type === "jewelry") {
                ...
                if (
                    ...
                    console.log(
                        `Skipping ${item.name}: appears to be ammunition`,
                    );
                    return;
                }
...
settingsBtn.className =
    "btn-parchment bg-gray-600 hover:bg-gray-700 text-white text-xs md:text-sm py-1 px-2";
...
const settingsActionsContainer = document.getElementById(
...
removePortraitBtn.id = "remove-portrait-btn";
removePortraitBtn.className =
    "btn-parchment bg-red-600 hover:bg-red-700 text-white w-full";
removePortraitBtn.innerHTML =
    '<i class="ra ra-cancel mr-2"></i>Remove Portrait';

resetProgressionBtn.id = "reset-progression-btn";
resetProgressionBtn.className =
    "btn-parchment bg-orange-600 hover:bg-orange-700 text-white w-full";
resetProgressionBtn.innerHTML =
    '<i class="ra ra-recycle mr-2"></i>Reset Progression';

illustrationToggle.id = "illustration-mode-toggle";
illustrationToggle.className =
    "btn-parchment bg-indigo-600 hover:bg-indigo-700 text-white w-full";
illustrationToggle.title = "Toggle illustration mode for scenery images";

richTextToggle.className =
    "btn-parchment rich-text-toggle bg-purple-600 hover:bg-purple-700 text-white w-full";
richTextToggle.title = "Toggle rich text styling for game messages";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const updateIllustrationButton = () => {
        if (isIllustrationModeActive) {
            ...
            illustrationToggleBtn.className =
                "btn-parchment bg-green-600 hover:bg-green-700 text-white w-full";
        } else {
    ...
    const donationButtonsContainer = document.getElementById(
    ...
    if (donateBtn && donationModal) {
        ...
        donationAmounts.forEach((amount) => {
            ...
            button.className =
                "btn-parchment bg-green-600 hover:bg-green-700 text-white text-lg";
        ...
        inventoryItemsDisplay.addEventListener("click", function (event) {
            ...
            if (button) {
                ...
                switch (action) {
                    ...
                    case "use":
                        if (itemIndex !== null)
                            ItemManager.useItem(player, itemIndex);
                        break;
                    case "equip":
                        if (itemIndex !== null)
                            ItemManager.equipItem(player, itemIndex);
                        break;
                    case "sell":
                        if (itemIndex !== null)
                            ItemManager.sellItem(player, itemIndex);
                        break;
                    case "drop":
                        if (itemIndex !== null)
                            ItemManager.dropItem(player, itemIndex);
                        break;
                    case "unequip":
                        if (slot) {
